...
from TAG.linkproppred.crime import Crime
...
if __name__ == "__main__":
    ...
    if args.emb_type == 'GPT-3.5-TURBO':
        npdata = np.load('crime_dataset/emb/review.npy')
        data['user', 'review', 'book'].edge_attr = torch.tensor(npdata).squeeze().float()
        npdata = np.load('crime_dataset/emb/edge_attr_book_genre.npy')
        ...
        data['book', 'description', 'genre'].edge_attr = torch.randn(num_descriptions, 256).squeeze().float()
    ...
    model = Model(hidden_channels=256, edge_dim=256, num_layers=2, model_type=args.model_type)
